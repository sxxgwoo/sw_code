...
void DeleteTree(BTreeNode * bt)  // binary tree의 모든 노드를 메모리에서 해제하는 함수 (무조건 Postorder Traversal) 밑에부터 없애야하므로
{
	if(bt == NULL)     // 현재 노드가 NULL이면 재귀 종료
		return;
	...
	printf("del tree data: %d \n", bt->data);   // 현재 노드의 데이터 출력 (삭제 로그)
	free(bt);    // 현재 노드의 메모리 해제
}
